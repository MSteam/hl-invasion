...
void CTankCam :: CamThink ( void )
{
	...
	// fonction appel�e tous les 1/20 de seconde
	...
	{
		while ( (int)m_skin > 8 )	//de 0 � 8 : 9 images
		{
			m_skin -= 8;
		}
...
void	CTankCam :: SetPlayerTankView ( BOOL setOn )
{
	...
	// bug des dommages � la sauvegarde
...
{
	//ne sert strictement � rien mais sans cette fonction think, les vecteurs vitesse ne veulent pas s'appliquer � mon tank
	pev->nextthink = pev->ltime + 0xFF;
}
...
int	CTankBSP :: TakeDamage ( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
	...
	{
		m_pTankModel->m_pCam->SetPlayerTankView(TRUE);	// rafraichissement de la vie c�t� client
...
//============================================
//	Le bone controller 0 correspond au d�placement haut-bas
//	il �quivaut � l axe X du joueur
//	le bone 1 est le d�placement gauche-droite
...
//	ALERT ( at_console, "playerdrivetank : %s\n", m_pPlayer->m_iDrivingTank == TRUE ? "TRUE" : "FALSE" ); 
	...
	if ( tr[0].vecEndPos == vecFrontRight && tr[1].vecEndPos == vecBackRight && tr[2].vecEndPos == vecBackLeft && tr[3].vecEndPos == vecFrontLeft )
	{
		...
		// pour combler la diff�rence de vitesse entre le bsp et le mdl
...
void CTank :: StopThink ( void )
{
	...
	m_pCam->SetThink ( &CTankCam::SUB_Remove );
...
void CTank :: Fire ( int canon )
{
	...
	// on applique nous-m�me les dommages - rayon : 250
